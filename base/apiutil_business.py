...
class RequestBaseBusiness:
    ...
    def specification_yaml(self, case_info):
        ...
        try:
            ...
            url = base_url + case_info["base_info"]["url"]
            allure.attach(url, f'接口地址：{url}')
            api_name = case_info["base_info"]["api_name"]
            allure.attach(api_name, f'接口名：{api_name}')
            method = case_info["base_info"]["method"]
            allure.attach(method, f'请求方法：{method}')
            headers = self.replace_load(case_info["base_info"]["headers"])
            allure.attach(str(headers), '请求头信息', allure.attachment_type.TEXT)
            try:
                cookies = self.replace_load(case_info["base_info"]["cookies"])
                allure.attach(str(cookies), 'Cookies', allure.attachment_type.TEXT)
            except:
                pass
            for tc in case_info["test_case"]:
