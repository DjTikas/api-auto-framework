...
class RequestBase:
    ...
    def specification_yaml(self, base_info, test_case):
        """
        接口请求处理基本方法
        :param base_info: yaml文件里面的base_info
        :param test_case: yaml文件里面的test_case
        :return:
        """
        try:
            ...
            allure.attach(api_name, f'请求头：{headers}', allure.attachment_type.TEXT)
            # 处理cookies
            cookies = None
...
if __name__ == '__main__':
    case_info = get_testcase_yaml(FILE_PATH['YAML'] + '/login.yaml')[0]
    print(case_info)
    RequestBase().specification_yaml(case_info[0], case_info[1])
